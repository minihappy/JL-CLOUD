...
// * @since 0.2.3
// */
//public final class FederatedIdentityAuthenticationEntryPoint implements AuthenticationEntryPoint {//联合身份的入口点,身份验证过会调用commence
//
...
//
//    @Override
//    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authenticationException) throws IOException, ServletException {//当身份验证时调用
//        String idp = request.getParameter("idp");
//        if (idp != null) {
//            ClientRegistration clientRegistration = this.clientRegistrationRepository.findByRegistrationId(idp);//获取客户端名称
//            if (clientRegistration != null) {//如果查询出来客户端不为空
//                String redirectUri = UriComponentsBuilder.fromHttpRequest(new ServletServerHttpRequest(request))
//                        .replaceQuery(null)
//                        .replacePath(this.authorizationRequestUri)//替换客户端uri为认证的uri
//                        .buildAndExpand(clientRegistration.getRegistrationId())//将以http:认证服务器+认证请求+客户端id的形式生成String字符串
//                        .toUriString();
//                this.redirectStrategy.sendRedirect(request, response, redirectUri);//跳转到重定向地址
//                return;
//            }
//        }

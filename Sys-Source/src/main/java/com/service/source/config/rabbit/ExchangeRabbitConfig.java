...
public class ExchangeRabbitConfig implements BeanPostProcessor {
    ...
    public TopicExchange rabbitmqTopicExchange() {
        //Topic交换机
        return new TopicExchange(RabbitMQConfig.TOPIC_EXCHANGE_ROLE_AUTHORITY_CHANGE);
    }
    ...
    public Queue rabbitmqTopicQueue_A() {
         ...
         * */
        return new Queue(RabbitMQConfig.TOPIC_ROLE_AUTHORITY_CHANGE_QUEUE_A, true, false, false);
    }
    ...
    public Queue rabbitmqTopicQueue_B() {
         ...
         * 4、autoDelete: 是否自动删除。也就是临时队列。当最后一个消费者断开连接后，会自动删除。
         * */
        return new Queue(RabbitMQConfig.TOPIC_ROLE_AUTHORITY_CHANGE_QUEUE_B, true, false, false);
    }

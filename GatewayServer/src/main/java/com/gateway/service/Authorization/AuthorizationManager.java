package com.gateway.service.Authorization;


import cn.hutool.core.convert.Convert;
import com.gateway.service.constant.AuthConstant;
import com.gateway.service.constant.RedisConstant;
...
//        authorities = authorities.stream().map(i -> i = AuthConstant.AUTHORITY_PREFIX + i).collect(Collectors.toList());
        authorities = authorities.stream().collect(Collectors.toList());//不加前缀，不需要重组角色
        //认证通过且角色匹配的用户可访问当前路径
        return mono
                //判断认证是否成功
